...
public class BaseEncryptInfo implements Serializable {
    ...
    private int uid = 0;//同城圈用户id，未登录为0
    private String username = "";//用户名
    private String cityid = "";//城市ID，格式是350200
    private String appversion = "";//同城圈版本号
    private int timestamp = 0;//时间戳 (每次请求的timestamp必需都不一样)
    private String device = "0";//设备ID ios为1 android为0
    private String mobile = "";//同城圈手机号
    private String udid = "";//手机号设备号

    private UserInfoEntity mUserInfoEntity;

    private BaseEncryptInfo(Context context) {
        params = new JSONObject();
        cityid = UFunTool.getCityId(context);
        mUserInfoEntity = AccountUtil.getUserInfoEntity(context);
        if (mUserInfoEntity != null && mUserInfoEntity.data != null) {
            token = mUserInfoEntity.data.token;
            uid = mUserInfoEntity.data.uid;
            username = mUserInfoEntity.data.username;
            mobile = mUserInfoEntity.data.mobile;
        }
        timestamp = (int) (System.currentTimeMillis() / 1000);
        device = AppHelper.getOSTypeNew();
        appversion = AppHelper.getVersionCode(context) + "";
        udid = AppHelper.getIMEI(context);
    }
    ...
    private static void refreshUserInfo(Context context) {
        UserInfoEntity user = AccountUtil.getUserInfoEntity(context);
    ...
    public String toString() {
        ...
        return "加密参数列表：BaseEncryptInfo[cityid = " + cityid + ",uid = " + uid + ",username = "
                ...
                + ",appversion = " + appversion + ",mobile = " + mobile + ",udid = " + udid + "]";
    }
