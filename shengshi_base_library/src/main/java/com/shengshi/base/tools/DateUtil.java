...
public final class DateUtil implements Serializable {
     ...
     * @throws ParseException
     */
    public static String dateFormat(String dateStr, String pattern) throws ParseException {
//		SimpleDateFormat formatter = new SimpleDateFormat(format);
//		java.sql.Date date = java.sql.Date.valueOf(sdate);
//		String dateString = formatter.format(date);

        Date date = getDateByString(dateStr, pattern);
        String dateString = getFormatDateTime(date, pattern);
        return dateString;
    }
     ...
     * @throws Exception
     */
    public static List<String[]> getCurrWeek() throws Exception {
        return getCurrWeek(getCurrentDate());
    }

    /**
     * 获得从当前日期开始到一个月后的，这一个月内的周一到周日的 日期列表
     *
     * @return
     * @throws Exception
     */
    public static List<String[]> getCurrWeekAfterMonth() throws Exception {
        List<String[]> result = new ArrayList<String[]>();
        String dataStr = getCurrentDate();
//		dataStr = "2014-10-11 10:22:22";
        List<String[]> currWeeks = getCurrWeek(dataStr);

        Calendar calendar = Calendar.getInstance();
        calendar.setTime(getDateFromString(dataStr, "yyyy-MM-dd"));
        calendar.add(Calendar.MONTH, 1);

        String endDate = getFormatDate(calendar.getTime());//下个月
        List<String[]> nextMonthWeeksAfterCurrent = getCurrWeek(endDate);

        calendar.set(Calendar.DAY_OF_MONTH, 1);
        String endDateOne = getFormatDate(calendar.getTime());//下个月一号
        List<String[]> nextMonthWeeks = getCurrWeek(endDateOne);

        int currSize = currWeeks.size();
        int nextSize = nextMonthWeeks.size();
        result.addAll(currWeeks);
        for (int index = 0; index < nextSize - currSize; index++) {
            if (!result.get(currSize - 1)[0].equals(nextMonthWeeks.get(index)[0])) {
                result.add(nextMonthWeeks.get(index));
            }
        }
        if (!result.get(result.size() - 1)[0].equals(nextMonthWeeksAfterCurrent.get(0)[0])) {
            result.add(nextMonthWeeksAfterCurrent.get(0));
        }
...
//		}
     ...
     * @throws Exception
     */
    public static List<String[]> getCurrWeek(String dataStr) throws Exception {
        ...
        if ((calendar.get(Calendar.DAY_OF_WEEK) - 1) != 1) {
            ...
            for (int index = 1; index < 7; index++) {
                ...
                str[index] = sdf.format(calendar.getTime());
            }
            list.add(str);
            //本周日加一，设置为下周开始时间
            calendar.add(Calendar.DATE, 1);
